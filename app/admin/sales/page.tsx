...
import { notifySaleStatusChanged, getStatusLabel } from "@/lib/notify"
...
function AdminSalesContent() {
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      // Deep link: abrir venta especifica si viene ?open=<id>
      const openId = searchParams.get("open")
      if (openId) {
        const found = response.sales.find((s: Sale) => s._id === openId)
        if (found) {
  ...
  const filterSales = () => {
    ...
    filtered = filtered.filter((sale) => {
      ...
      // Todos los demas estados (pending, pending_signature, pending_appointment, cancelled): usar fecha de creacion
  ...
  const handleOpenCostsDialog = (sale: Sale) => {
    ...
    const currentSupervisorId = sale.supervisorId
      ? (typeof sale.supervisorId === "object" ? sale.supervisorId._id : sale.supervisorId)
      : ""
    setCostsData({
  ...
  const handleDeleteSale = async () => {
    ...
    setIsDeleting(true)
    try {
      await salesAPI.deleteSale(token, saleToDelete._id)
      toast({
        title: "Venta eliminada",
        description: `La venta de ${saleToDelete.customerInfo.name} fue eliminada.`,
      })
  ...
  const handleUpdateCosts = async () => {
    ...
    try {
      // Si cambio el vendedor, asignar primero (el backend auto-setea sellerCommissionPaid)
      ...
      // Actualizar costos despues (el valor manual del admin prevalece sobre el auto-set)
      ...
      // Si cambio el supervisor, actualizar supervisorId
      const originalSupervisorId = selectedSale.supervisorId
        ? (typeof selectedSale.supervisorId === "object" ? selectedSale.supervisorId._id : selectedSale.supervisorId)
        : ""
      if (costsData.newSupervisorId !== originalSupervisorId) {
        try {
          await salesAPI.update(token, selectedSale._id, {
            supervisorId: costsData.newSupervisorId || null,
          } as any)
        } catch (supError) {
          console.error("Error assigning supervisor:", supError)
        }
      }
...
const result = await salesAPI.updateStatus(
      ...
      // Enviar notificacion push al vendedor
      // sellerId puede ser string o objeto poblado
      const sellerIdForNotify = typeof selectedSale.sellerId === 'object' 
        ? (selectedSale.sellerId as { _id: string })._id 
        : selectedSale.sellerId
      notifySaleStatusChanged({
        saleId: selectedSale._id,
        sellerId: sellerIdForNotify,
        customerName: selectedSale.customerInfo.name,
        ...
        <Dialog open={isCostsDialogOpen} onOpenChange={setIsCostsDialogOpen}>
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              {/* Selector de vendedor — solo si la venta no tiene supervisor como dueño */}
              {(!selectedSale?.supervisorId || selectedSale?.sellerId) && (
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground">Asignar Vendedor</label>
                  <Select
                    value={costsData.newSellerId}
                    onValueChange={(value) => {
                      const seller = users.find(u => u._id === value)
                      setCostsData(prev => ({
                        ...prev,
                        newSellerId: value,
                        sellerCommissionPaid: seller?.fixedCommissionPerSale != null
                          ? String(seller.fixedCommissionPerSale)
                          : prev.sellerCommissionPaid,
                      }))
                    }}
                  >
                    ...
                    <SelectContent>
                      {users.filter(u => u.role === "seller").map((user) => (
                        <SelectItem key={user._id} value={user._id}>
                          {user.name}{user.fixedCommissionPerSale ? ` — $${user.fixedCommissionPerSale.toLocaleString("es-AR")}` : ""}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">Asignar Supervisor</label>
                <Select
                  value={costsData.newSupervisorId || "none"}
                  onValueChange={(value) => setCostsData(prev => ({ ...prev, newSupervisorId: value === "none" ? "" : value }))}
                >
                  <SelectTrigger className="bg-secondary/50">
        ...
        <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Estás por eliminar la venta de{" "}
                <span className="font-semibold text-foreground">
                  {saleToDelete?.customerInfo.name}
                </span>{" "}
                (DNI: {saleToDelete?.customerInfo.dni}). Esta acción no se puede deshacer.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
