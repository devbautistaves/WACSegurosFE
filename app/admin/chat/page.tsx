...
import { Send, Users, MessageSquare, Shield, UserCheck, UserCircle, MessageCircle, Headphones, Trash2, CheckSquare, Square, X as XIcon } from "lucide-react"
...
function AdminChatContent() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      // Deep link: abrir sala especifica si viene ?room=<roomId>
      const roomId = searchParams.get("room")
      if (roomId && groupRes.chatRoom) {
        if (groupRes.chatRoom._id === roomId) {
          setSelectedRoom(groupRes.chatRoom)
          setSelectedChatType("group")
        } else {
          // Buscar en salas privadas
          try {
            const allPrivate = await chatAPI.getPrivateChats(token)
            const targetRoom = allPrivate.chatRooms?.find((r: ChatRoom) => r._id === roomId)
            if (targetRoom) {
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    if (!token) return
    try {
      await chatAPI.deleteMessage(token, selectedRoom._id, messageId)
  ...
  const handleDeleteSelected = async () => {
    if (!selectedRoom || selectedMsgIds.size === 0) return
    const token = localStorage.getItem("token")
    if (!token) return
    setIsDeletingMsgs(true)
    try {
      await chatAPI.deleteMessages(token, selectedRoom._id, Array.from(selectedMsgIds))
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case "support":
        return { label: "Soporte", color: "bg-green-500/20 text-green-400 border-green-500/30" }
      default:
                ...
                {supportUsers.length > 0 && (
                  <div className="p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-medium text-green-400">Soporte ({supportUsers.length})</span>
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {selectMode ? (
                        <>
                          {selectedMsgIds.size > 0 && (
                            <Button
                              ...
                              disabled={isDeletingMsgs}
                              className="gap-1"
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                              Eliminar ({selectedMsgIds.size})
                            </Button>
                          )}
                        ...
                        <Button size="sm" variant="ghost" onClick={toggleSelectMode} className="text-muted-foreground">
                      ...
                      return (
                        <div key={msg._id}>
                          ...
                          <div
                            ...
                            onClick={selectMode ? () => toggleMessageSelect(msg._id) : undefined}
                          >
                            {/* Checkbox en modo selección */}
                            {selectMode && (
                              <div className={cn("flex items-center mt-2 shrink-0", ownMessage && "order-last")}>
                                {isSelected
                            ...
                            <div className={cn("flex items-end gap-1", ownMessage && "flex-row-reverse")}>
                              <div
                                className={cn(
                              ...
                              >
                                ...
                                <p className={cn("text-xs mt-1", ownMessage ? "text-primary-foreground/70" : "text-muted-foreground")}>
                                  {formatTime(msg.createdAt)}
                                </p>
                              </div>
                              {/* Trash icon en hover (solo fuera de modo selección) */}
                              {!selectMode && (
                                <button
                                  onClick={(e) => { e.stopPropagation(); handleDeleteMessage(msg._id) }}
                                  className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-destructive/10 text-destructive shrink-0 mb-1"
