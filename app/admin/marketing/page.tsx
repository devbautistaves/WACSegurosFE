...
const CATEGORY_CONFIG: Record<MaterialCategory, { label: string; description: string; icon: typeof Folder; color: string }> = {
  ...
  planes_vigentes: {
    ...
    description: "Planes y tarifas vigentes para ofrecer a clientes",
    icon: Globe,
    color: "green",
  },
}
...
export default function AdminMarketingPage() {
  ...
  const loadMaterials = async () => {
    try {
      ...
      if (response.success) setMaterials(response.materials)
    } catch (error) {
  ...
  const handleUpload = async () => {
    if (!newMaterial.name || !newMaterial.file || !selectedCategory) {
      toast({ title: "Error", description: "Completa el nombre y selecciona un archivo", variant: "destructive" })
      return
    }
    try {
      setIsUploading(true)
      ...
      const response = await materialsAPI.upload(
        token,
        { category: selectedCategory, name: newMaterial.name, description: newMaterial.description },
        newMaterial.file
      )
      if (response.success) {
        toast({ title: "Material subido", description: "El material se ha subido correctamente" })
  ...
  const getMaterialsByCategory = (category: MaterialCategory) => materials.filter(m => m.category === category)
      ...
      <DashboardLayout requiredRole="admin">
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">Material de marketing y capacitacion</p>
          </div>
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Hacé clic en Subir Archivo para agregar nuevo material</CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <Card key={material._id} className="overflow-hidden group">
                      <div className="aspect-video bg-secondary flex items-center justify-center relative">
                        {material.fileType === "image" ? (
                          <img src={fileUrl} alt={material.name} className="w-full h-full object-cover" />
                        ) : (
                          getFileIcon(material.fileType)
                        )}
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                          <a href={`${viewUrl}?token=${token}`} target="_blank" rel="noopener noreferrer">
                            <Button size="sm" variant="secondary">
                              <Eye className="h-4 w-4 mr-1" />
                              Ver
                            </Button>
                          </a>
                          <a href={`${downloadUrl}?token=${token}`} target="_blank" rel="noopener noreferrer">
                            <Button size="sm" variant="secondary">
                          ...
                          <Button
                            size="sm"
                            variant="destructive"
                            onClick={(e) => { e.preventDefault(); handleDelete(material._id) }}
                          >
      ...
      <Dialog open={showUploadModal} onOpenChange={setShowUploadModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Subí un archivo para agregarlo a esta carpeta</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) setNewMaterial(prev => ({ ...prev, file, name: prev.name || file.name.replace(/\.[^/.]+$/, "") }))
                }}
              />
              {newMaterial.file ? (
                <div className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setNewMaterial(prev => ({ ...prev, file: null }))}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ) : (
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpload} disabled={isUploading}>
              {isUploading ? (
                <><Spinner className="h-4 w-4 mr-2" />Subiendo...</>
              ) : (
