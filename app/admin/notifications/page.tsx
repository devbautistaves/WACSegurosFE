...
import {
  Bell, BellOff, Check, CheckCheck, ShoppingCart, Info, Mail,
...
const TYPE_CONFIG: Record<string, { label: string; icon: React.ComponentType<{ className?: string }>; color: string }> = {
  ...
  new_sale: { label: "Nueva venta", icon: Mail, color: "text-green-500 bg-green-500/10" },
}
...
function AdminNotificationsInner() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      // Auto-abrir notificacion si viene ?open=ID
        ...
        <Card className="border-border/50 bg-card/50">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Todas las Notificaciones ({filtered.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {filtered.length === 0 ? (
