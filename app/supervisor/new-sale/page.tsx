...
export default function NewSalePage() {
  ...
  const shortDescription = formData.description
    ...
    const saleData = {
      ...
      ...(selectedSeller && { sellerId: selectedSeller._id }),
    ...
    // Enviar al backend
    try {
      const result = await salesAPI.create(token, saleData as any)
      
      // Auto-setear comisión del vendedor desde su comisión fija
      if (result.sale?._id && selectedSeller?.fixedCommissionPerSale) {
        try {
          await salesAPI.updateCosts(token, result.sale._id, {
            sellerCommissionPaid: selectedSeller.fixedCommissionPerSale,
          })
        } catch (costError) {
          console.error("Error setting seller commission:", costError)
        }
      }
                  ...
                  <Field>
                    ...
                    <div className="space-y-2">
                      <div className="grid gap-2 sm:grid-cols-2">
                        ...
                        >
                          <p className="text-sm font-semibold">Venta propia</p>
                        ...
                        {sellers.map(seller => (
                          ...
                          >
                            ...
                            <p className="text-xs text-muted-foreground">
                              {seller.fixedCommissionPerSale
                                ? `Comisión fija: ${formatCurrency(seller.fixedCommissionPerSale)}`
                                : "Sin comisión fija"}
                            </p>
                          </div>
                        ))}
                      </div>
                      {selectedSeller && selectedSeller.fixedCommissionPerSale && (
                        <p className="text-xs text-amber-400">
                          Se descontará {formatCurrency(selectedSeller.fixedCommissionPerSale)} de tu comisión por esta venta.
                        </p>
                      )}
                    </div>
                  </Field>
