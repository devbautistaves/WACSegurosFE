...
export default function SupervisorDashboardPage() {
  ...
  const totalSellerCommissions = installedSales.reduce((acc, sale) => {
    return acc + (Number(sale.sellerCommissionPaid) || 0)
  }, 0)
                  ...
                  {salesThisMonth.slice(0, 10).map((sale) => (
                    <tr key={sale._id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="py-3 px-4 hidden md:table-cell">
                        {sale.sellerCommissionPaid && sale.sellerCommissionPaid > 0 ? (
                          <div>
                            <p className="text-sm text-foreground">{sale.sellerName}</p>
                            <p className="text-xs text-amber-400">-{formatCurrency(sale.sellerCommissionPaid)}</p>
                          </div>
                        ) : (
                          <span className="text-muted-foreground text-sm">Propia</span>
                        )}
                      </td>
